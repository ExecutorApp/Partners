...
import CardMenu from './3.CardMenu02';
...
import ModalFilters from './2.ModalFilter02';
import NewAppointment01 from './4.NewSale-Custome';
import NewAppointmentProducts from './6.NewSale-Products';
import NewAppointmentActivities from './7.NewSale-Activities';
import NewAppointmentAgendaTypes from './8.NewSale-TypeOfFees';
...
const APPOINTMENTS_STORAGE_KEY = 'partners.sales.items';
...
const SalesHomeScreen: React.FC = () => {
  ...
  const openAppointmentDetails = (apt: AppointmentItem) => {
    try {
      ...
      navigation.navigate(ScreenNames.SchedulingDetailsMain02 as any, { appointment: apt } as any);
    } catch (e) {
      ...
      >
        ...
        <View style={styles.rightContent}>
          <View style={styles.rightHeader}>
            <Text style={styles.personName}>{apt.client || 'Nova venda'}</Text>
      ...
      {!showMonthCalendar && (
        <View style={styles.actionsRow}>
          ...
          <TouchableOpacity style={[styles.newScheduleButton, styles.actionButtonMarginFix]} onPress={handleNewSchedule}>
            ...
            <Text style={styles.newScheduleText}>Nova venda</Text>
          </TouchableOpacity>
        </View>
      )}
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <View style={styles.emptyTitleWrapper}>
        ...
        <Text style={styles.emptyTitle}>encontrada!</Text>
      </View>
      <View style={styles.emptySubtitleWrapper}>
        <Text style={styles.emptySubtitle}>{'Para realizar uma nova venda, toque '}</Text>
        <Text style={styles.emptySubtitle}>{'no bot\u00E3o azul "Nova venda"'}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <ModalShell
        ...
        goToStep={(n: number) => setNewAppointmentStep(n as 1 | 2 | 3 | 4 | 5 | 6 | 7)}
        ...
        selectedAgendaType={selectedAgendaType}
        onSelectAgendaType={(id: string | null) => setSelectedAgendaType(id as 'personal' | 'shared' | null)}
        editing={isEditingSchedule}
        onSchedule={({ date, slots, client }: { date: Date; slots: SlotPair[]; client?: { id: string; photoKey?: string; photoUri?: string | null } | null }) => handleSchedule(date, slots, client ?? null)}
      />
