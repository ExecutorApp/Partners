...
type FeeItem = { id: string; number: string; name: string; value: number };
...
const NewAppointmentAgendaTypes: React.FC<NewAppointmentAgendaTypesProps> = ({ visible, onClose, onBack, onNext, onSelectAgendaType, summaries, onSelectStep, onUpdateSummary, maxAccessibleStep = 1, transitionDirection = 'forward', embedded = false, registerNextHandler, onCanProceedChange }) => {
  ...
  const agendaTypes: FeeItem[] = useMemo(() => [
    { id: 'none', number: '', name: 'Nenhum', value: 0 },
    { id: 'prolabore', number: '01', name: 'Prolabore', value: 800 },
    { id: 'prolabore_success', number: '02', name: 'Prolabore + Éxito', value: 1000 },
    { id: 'success', number: '03', name: 'Éxito', value: 1600 },
  ], []);
  ...
  const isNextDisabled = selectedType === null || selectedType === 'none';
  ...
  useEffect(() => {
    ...
    let fromSummary: string | null = null;
    if (summ.includes('nenhum')) fromSummary = 'none';
    else if (summ.includes('prolabore') && (summ.includes('éxito') || summ.includes('exito'))) fromSummary = 'prolabore_success';
    else if (summ.includes('prolabore')) fromSummary = 'prolabore';
    else if (summ.includes('éxito') || summ.includes('exito')) fromSummary = 'success';
    if (fromSummary) {
    ...
    onSelectAgendaType?.('none');
    onUpdateSummary?.(5, 'Nenhum');
  }, [visible]);
        ...
        <View style={styles.contentFlex}>
          ...
          <View style={styles.introContainer}>
            <View style={styles.centerBlock}>
              <Text style={styles.sectionTitle}>Tipo de Honorários</Text>
            ...
            {agendaTypes.map((t, idx) => (
              <View key={t.id}>
                <TouchableOpacity style={styles.productRow} activeOpacity={1} onPress={() => handleSelectType(t.id)} accessibilityRole="button" accessibilityLabel={`Selecionar ${t.name}`}>
                  <TouchableOpacity style={styles.selectIconTouch} onPress={() => handleSelectType(t.id)} accessibilityRole="button" accessibilityLabel={`Selecionar ${t.name}`} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                    <RadioIcon selected={selectedType === t.id} />
                  </TouchableOpacity>
                  {t.number ? <View style={styles.numberBox}><Text style={styles.numberText}>{t.number}</Text></View> : null}
                  <View style={styles.feeInfo}>
                    <Text style={styles.feeName}>{t.name}</Text>
                    {t.value > 0 ? <Text style={styles.feeValue}>{formatBRL(t.value)}</Text> : null}
                  </View>
                </TouchableOpacity>
