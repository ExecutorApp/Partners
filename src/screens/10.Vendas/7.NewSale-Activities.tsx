...
export default function NewAppointmentActivities({ visible, onClose, onBack, onNext, onSelectActivity, summaries, onSelectStep, onUpdateSummary, maxAccessibleStep = 1, transitionDirection = 'forward', embedded = false, registerNextHandler, onCanProceedChange }: NewAppointmentActivitiesProps) {
  ...
  const [completedIds, setCompletedIds] = useState<Set<string>>(new Set(['a1', 'a2']));
  const activities = useMemo(
    () => ([
      { id: 'a1', name: 'Reunião 01', price: 800 },
      { id: 'a2', name: 'Reunião 02', price: 1200 },
      { id: 'a3', name: 'Reunião 03', price: 450000 },
      { id: 'a4', name: 'Reunião 04', price: 500 },
      { id: 'a5', name: 'Reunião 05', price: 500 },
      { id: 'a6', name: 'Reunião 06', price: 500 },
      { id: 'a7', name: 'Reunião 07', price: 500 },
  ...
  useEffect(() => {
    ...
    const selectedIsValid = !!selectedActivity && filtered.some(a => a.id === selectedActivity) && !completedIds.has(selectedActivity);
    if (!selectedIsValid) {
      const firstId = getFirstNonCompletedId(filtered, completedIds) || null;
  ...
  useEffect(() => {
    if (visible) {
      const firstId = getFirstNonCompletedId(activities, completedIds) || null;
      setSelectedActivity(firstId);
      const found = activities.find(a => a.id === firstId);
      if (found) {
        onSelectActivity?.(firstId);
        onUpdateSummary?.(4, found.name);
      } else {
        onSelectActivity?.(null);
        onUpdateSummary?.(4, '');
      }
    }
  }, [visible, completedIds, activities]);
  ...
  if (embedded) {
    return (
      <SlideInView visible={visible} direction={transitionDirection} style={{ alignSelf: 'stretch', flex: 1 }}>
        ...
        <ScrollView style={styles.list} contentContainerStyle={{ paddingVertical: 6 }} showsVerticalScrollIndicator={false}>
          <View style={styles.contentFlex}>
            {filtered.map((act, idx) => {
              const isCompleted = completedIds.has(act.id);
              const isSelected = selectedActivity === act.id;
              const globalIdx = activities.findIndex(a => a.id === act.id);
              const displayNumber = (globalIdx + 1).toString().padStart(2, '0');
              return (
                <React.Fragment key={act.id}>
                  ...
                  >
                    <TouchableOpacity
                      ...
                      accessibilityLabel={isCompleted ? 'Atividade concluída' : 'Selecionar atividade'}
                      hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                    >
                      {isCompleted ? <GreenCheckIcon /> : <RadioIcon selected={isSelected} />}
                    </TouchableOpacity>
                    <View style={styles.numberBox}><Text style={styles.numberText}>{displayNumber}</Text></View>
                    <View style={styles.activityInfo}>
                      <Text style={[styles.activityName, isCompleted ? styles.activityNameStriked : null]}>{act.name}</Text>
                      <Text style={[styles.activityValue, isCompleted ? styles.activityValueStriked : null]}>{formatBRL(act.price ?? 0)}</Text>
                    </View>
                  </TouchableOpacity>
                  {idx < filtered.length - 1 ? <View style={styles.listDivider} /> : null}
                </React.Fragment>
              );
            })}
          </View>
