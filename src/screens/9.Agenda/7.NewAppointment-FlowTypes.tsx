...
interface NewAppointmentFlowTypesProps {
  visible: boolean;
  onClose: () => void;
  onBack: () => void;
  onNext?: (flowTypeId: 'guided' | 'free') => void;
  onSelectFlowType?: (flowTypeId: 'guided' | 'free' | null) => void;
...
// Ícone de busca — mesmo de 4.NewAppointment01
...
// Radio — mesmo padrão visual de 4.NewAppointment01
...
type FlowType = { id: 'guided' | 'free'; name: string; description: string };

const NewAppointmentFlowTypes: React.FC<NewAppointmentFlowTypesProps> = ({ visible, onClose, onBack, onNext, onSelectFlowType, summaries, onSelectStep, onUpdateSummary, maxAccessibleStep = 1, transitionDirection = 'forward', embedded = false, registerNextHandler, onCanProceedChange }) => {
  const [fullFlowVisible, setFullFlowVisible] = useState<boolean>(false);
  const [search, setSearch] = useState('');
  const [selectedFlowType, setSelectedFlowType] = useState<'guided' | 'free' | null>(null);

  const flowTypes: FlowType[] = useMemo(() => [
    { id: 'guided', name: 'Guiado', description: 'Siga a ordem predefinida de reuniões do sistema' },
    { id: 'free', name: 'Livre', description: 'Você define manualmente cada reunião' },
  ], []);

  const filteredFlowTypes = useMemo(() => {
    const sText = normalizeText(search);
    if (!sText) return flowTypes;
    return flowTypes.filter((f) => normalizeText(f.name).includes(sText) || normalizeText(f.description).includes(sText));
  }, [search, flowTypes]);

  const isNextDisabled = selectedFlowType === null;

  // Inicialização: respeita resumo salvo; aplica padrão somente se não houver seleção prévia
  useEffect(() => {
    if (!visible) return;
    if (selectedFlowType) return;
    const summ = (summaries?.[3] ?? '').toLowerCase();
    const fromSummary = summ.includes('livre') ? 'free' : summ.includes('guiado') ? 'guided' : null;
    if (fromSummary) {
      setSelectedFlowType(fromSummary);
      onSelectFlowType?.(fromSummary);
      return;
    }
    setSelectedFlowType('guided');
    onSelectFlowType?.('guided');
    onUpdateSummary?.(3, 'Guiado');
  }, [visible]);

  const handleSelectFlow = (id: 'guided' | 'free') => {
    setSelectedFlowType(id);
    onSelectFlowType?.(id);
    const found = flowTypes.find((f) => f.id === id);
    if (found) onUpdateSummary?.(3, found.name);
  };
  ...
  useEffect(() => {
    if (registerNextHandler) {
      registerNextHandler(() => {
        if (!isNextDisabled && selectedFlowType) {
          onNext?.(selectedFlowType);
        }
      });
    }
  }, [registerNextHandler, isNextDisabled, selectedFlowType, onNext]);
        ...
        <View style={styles.contentFlex}>
          <View style={styles.introContainer}>
            <View style={styles.centerBlock}>
              <Text style={styles.sectionTitle}>Tipos de fluxo</Text>
              <Text style={styles.sectionSubtitle}>Selecione um fluxo para continuar</Text>
            </View>
            <View style={styles.stepsRow}>
              {Array.from({ length: 7 }).map((_, i) => (
                <View key={i} style={[styles.stepDot, i <= 2 ? styles.stepDotActive : styles.stepDotInactive]} />
              ))}
            </View>
          </View>
          <ScrollView style={styles.list} showsVerticalScrollIndicator={false}>
            {filteredFlowTypes.map((f, idx) => (
              <View key={f.id}>
                <TouchableOpacity style={styles.flowRow} activeOpacity={1} onPress={() => handleSelectFlow(f.id)} accessibilityRole="button" accessibilityLabel={`Selecionar ${f.name}`}>
                  <View style={styles.radioWrap}>
                    <RadioIcon selected={selectedFlowType === f.id} />
                  </View>
                  <View style={styles.flowInfo}>
                    <Text style={styles.flowName}>{f.name}</Text>
                    <Text style={styles.flowDescription}>{f.description}</Text>
                  </View>
                </TouchableOpacity>
                {idx < filteredFlowTypes.length - 1 ? <View style={styles.listDivider} /> : null}
              </View>
            ))}
          </ScrollView>
        </View>
  ...
  return (
    <>
      ...
      <FullFlow visible={fullFlowVisible} onClose={() => setFullFlowVisible(false)} currentStep={3} summaries={summaries} onSelectStep={onSelectStep} maxAccessibleStep={maxAccessibleStep} />
    </>
  );
};
...
const styles = StyleSheet.create({
  ...
  radioWrap: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    // pequeno ajuste para alinhar verticalmente com o título (Guiado/Livre)
    marginTop: 2,
  },
  flowInfo: { flex: 1 },
  flowName: {
...
export default NewAppointmentFlowTypes;