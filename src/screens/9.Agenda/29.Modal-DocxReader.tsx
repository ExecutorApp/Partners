...
const PaginationColumn: React.FC<PaginationColumnProps> = React.memo(({
  currentPage,
  totalPages,
  onPageSelect,
  onPreviousPage,
...
// ============================================================================
// GERADOR DO SRCDOC PARA DOCX
// ============================================================================

const generateFullWidthSrcDoc = (dataSource: string, isBase64: boolean = false): string => {
  const pageBg = PAGE_CONFIG.PAGE_BG;
  const paddingH = PAGE_CONFIG.PAGE_PADDING_H;
  const paddingV = PAGE_CONFIG.PAGE_PADDING_V;
  const separatorHeight = PAGE_CONFIG.PAGE_SEPARATOR_HEIGHT;
  const separatorColor = PAGE_CONFIG.PAGE_SEPARATOR_COLOR;
  const toolbarHeight = PAGE_CONFIG.TOOLBAR_HEIGHT;
...
<head>
  ...
  <style>
    * {
    ...
    .docx-wrapper, .docx-wrapper *, #viewer, #container {
    ...
    [style*="font-weight: 100"], .fw-100 { font-weight: 100 !important; }
    [style*="font-weight: 200"], .fw-200 { font-weight: 200 !important; }
    [style*="font-weight: 300"], .fw-300 { font-weight: 300 !important; }
    [style*="font-weight: 400"], .fw-400 { font-weight: 400 !important; }
    [style*="font-weight: 500"], .fw-500 { font-weight: 500 !important; }
    [style*="font-weight: 600"], .fw-600 { font-weight: 600 !important; }
    [style*="font-weight: 700"], .fw-700 { font-weight: 700 !important; }
    [style*="font-weight: 800"], .fw-800 { font-weight: 800 !important; }
    [style*="font-weight: 900"], .fw-900 { font-weight: 900 !important; }
    ...
    .docx-wrapper .docx > section {
      ...
      padding-bottom: ${paddingV + separatorHeight}px !important;
    ...
    .docx-wrapper .docx > section::after {
      content: '';
      display: block;
      position: absolute;
      left: 0;
    ...
    .docx-wrapper .docx > section:last-child::after {
      display: none;
    }
...
<body>
  ...
  (function() {
    ...
    var TOOLBAR_HEIGHT = ${toolbarHeight};
    ...
    // Função para mostrar as barras - adiciona padding E AUMENTA scrollTop para compensar
    window.showBars = function() {
      currentTopOffset = TOOLBAR_HEIGHT;
      viewer.style.paddingTop = TOOLBAR_HEIGHT + 'px';
      
      // AUMENTAR scrollTop para manter o mesmo conteúdo visível
      viewer.scrollTop = viewer.scrollTop + TOOLBAR_HEIGHT;
      
      setTimeout(calculatePageOffsets, 50);
    };
    
    // Função para esconder as barras - remove padding e DIMINUI scrollTop
    window.hideBars = function() {
      var previousOffset = currentTopOffset;
      currentTopOffset = 0;
      viewer.style.paddingTop = '0px';
      
      // DIMINUIR scrollTop para manter o mesmo conteúdo visível
      viewer.scrollTop = Math.max(0, viewer.scrollTop - previousOffset);
      
      setTimeout(calculatePageOffsets, 50);
    };
    
    // Detecção de página atual durante scroll
    function detectCurrentPage() {
      if (pageOffsets.length === 0 || isNavigating) return;
      
      var scrollTop = viewer.scrollTop;
      var viewerHeight = viewer.clientHeight;
      var viewCenter = scrollTop + (viewerHeight * 0.3);
      var newPage = 1;
    ...
    // Navegação instantânea para página
    ...
    function calculatePageOffsets() {
      pageOffsets = [];
      var sections = container.querySelectorAll('.docx-wrapper > section, .docx-wrapper .docx > section');
      
      if (sections.length === 0) {
        var docx = container.querySelector('.docx-wrapper, .docx');
        if (docx) {
          pageOffsets.push({ start: 0, end: docx.offsetHeight });
        }
        totalPages = 1;
      } else {
    ...
    function renderFromBlob(blob) {
      ...
      var loadingEl = loading.cloneNode(true);
      container.innerHTML = '';
      ...
      docx.renderAsync(blob, container, null, opts)
        ...
        .catch(function(err) {
          ...
          showError('Falha ao renderizar o documento');
        });
    }
      ...
      if (isBase64) {
        ...
        fetch(dataSource)
          ...
          .catch(function(err) {
            ...
            var proxy = 'https://cors.isomorphic-git.org/' + dataSource;
            fetch(proxy)
              .then(function(res) {
                if (!res.ok) throw new Error('HTTP ' + res.status);
                return res.blob();
              ...
              .then(renderFromBlob)
              .catch(function(err2) {
                console.error('Erro com proxy:', err2);
                showError('Falha ao carregar o documento');
              });
          });
      }
...
// ============================================================================
const ModalDocxReader: React.FC<Props> = ({ visible, onClose, title, uri, onEdit, fileRef }) => {
  const [barsVisible, setBarsVisible] = React.useState(false);
  const [currentPage, setCurrentPage] = React.useState(1);
  const [totalPages, setTotalPages] = React.useState(1);
  const iframeRef = React.useRef<HTMLIFrameElement>(null);

  const safeTitle = title || 'Documento Word';

  const [inlineDocxBase64, setInlineDocxBase64] = React.useState<string | null>(null);
  ...
  React.useEffect(() => {
    ...
    async function toBase64FromFile(f: File) {
      try {
        ...
        if (!cancelled) setInlineDocxBase64(b64);
      } catch (err) {
        if (!cancelled) setInlineDocxBase64(null);
      }
    }
  ...
  const iframeSrcDoc = React.useMemo(() => {
    ...
    if (inlineDocxBase64) {
      return generateFullWidthSrcDoc(inlineDocxBase64, true);
    }
    
    if (uri && /^https?:\/\//i.test(uri)) {
      return generateFullWidthSrcDoc(uri, false);
    }
    
    return undefined;
  }, [uri, inlineDocxBase64]);

  // Chamar showBars ou hideBars no iframe quando barsVisible mudar
  React.useEffect(() => {
    ...
    try {
      ...
      if (iframe && iframe.contentWindow) {
        if (barsVisible) {
          if ((iframe.contentWindow as any).showBars) {
            (iframe.contentWindow as any).showBars();
          }
        } else {
          if ((iframe.contentWindow as any).hideBars) {
            (iframe.contentWindow as any).hideBars();
          }
        }
      }
    } catch (e) {}
  ...
  const handlePrint = () => {
    if (Platform.OS === 'web' && iframeSrcDoc) {
      const w = window.open('', '_blank');
      if (w) {
        w.document.open();
        w.document.write(iframeSrcDoc);
        w.document.close();
      } else {
  ...
  const handleDownload = () => {
    if (Platform.OS === 'web' && uri) {
      ...
      a.href = uri;
      a.download = `${safeTitle.replace(/\s+/g, '-')}.docx`;
        ...
        <View style={styles.documentArea}>
          {Platform.OS === 'web' ? (
            ...
            <View style={styles.mobileBox}>
              <Text style={styles.paragraphMuted}>
                Visualização embutida de DOCX não disponível.{'\n'}
...
export default ModalDocxReader;
